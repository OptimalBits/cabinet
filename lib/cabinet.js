...
exports = module.exports = function static(root, options, virtuals){
  var cache;
  ...
  if(options.cache){
    ...
    cache.on('remove', function(path, entry){
      for(var i=0;i<entry.watcher.length;i++){
        entry.watcher[i].close();
      }
    });
  }
  
  return function static(req, res, next) {	
    send(req, res, next, options, cache, virtuals);
  };
};
...
var send = exports.send = function(req, res, next, options, cache, virtuals){
  options = options || {};
  ...
  if(!tryCache(req, res, path)){
    ...
    fs.stat(path, function(err, stat){
      ...
      Filter.applyFilters(filters, req, res, next, path, options, function(err, data){
        if(err || !data){
   ...
   * @api private
   */
  function setResponseHeaders(res, stat){
    ...
    if (!res.getHeader('content-type')) {
      var type = mime.lookup(path), charset = mime.charsets.lookup(type);
  ...
  function cacheFile(data, headers, path, files){
    ...
    if(data instanceof Stream){
      ...
      data.on('end', function(){
        entry.complete = true;
        entry.watcher = removeChangedFiles(cache, files || path);
      }); 
    } else {
      entry.push(data);
      entry.complete = true;
    ...
    if(virtuals && virtuals[path]){
      if(!tryCache(req, res, path)){
        virtuals[path].call(this, function(err, data, files){
          setResponseHeaders(res, {size:data.length, mtime:new Date()});
          if(cache){
            entry = cacheFile(data, res._headers, path, files);
          }else{
            entry = [];
  ...
  function removeChangedFiles(cache, files){
    ...
    for(i=0;i<files.length;i++){
      ...
      try{
        ...
        console.log("Error starting to watch file:"+path+" "+err);
      }
    }
    return watchers;
  }

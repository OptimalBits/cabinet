...
var send = exports.send = function(req, res, next, options, cache, virtuals, watcher, metaCache){
  ...
  metaCache.get(path, !options.cache).then(function(meta){
    ...
    if(!sendFromCache(req, res, path)){
      ...
      Filter.applyFilters(filters, req, res, next, path, options, function(err, data, deps){
        ...
        //
        if(cache && !req.headers.range && chunkSize < options.cache.maxSize){
          ...
          cacheFile(data, chunkSize, res._headers, path);
        }
      });
    }
  }, function(err){
  ...
  function cacheFile(data, length, headers, path){
  ...
  function cacheStream(data, entry, length){
    function end(closed){
      if(entryLength(entry) === length){
        entry.resolve(entry);  
      }else{
        if(closed){
          error(Error("Data closed too early"))
        }else{
          error(Error("Invalid entry length"));
        }
      }
    }
  ...
  function entryLength(entry){
    var length = 0, i = 0, data;
    while(data = entry.data[i++]){
      length += data.length;
    }
    return length;
  }
  ...
  function sendFromCache(req, res, path){
    ...
    if (hit && !req.headers.range) {
      hit.promise.then(function(entry){
        ...
        winston.error("cache hit but invalid entry:", path, err);
        respond(res, 500, err.msg);        
      });
      return true;
    }else{
